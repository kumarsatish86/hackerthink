    ...
    <DetailSection id="datasets" title="Training Data" description="Datasets used to train this model">
      {trainingData.length > 0 ? (
          ...
          {trainingData.map((ds) => (
            <Card key={ds.id} className="p-4">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <h3 className="font-semibold text-[var(--m-text)]">{ds.dataset_name}</h3>
              ...
              {ds.download_url && (
                ...
                >
                  <FaExternalLinkAlt className="h-3 w-3" /> View Dataset
                </a>
              )}
            </Card>
          ))}
        ...
        <Card className="p-6 text-center">
          ...
          <p className="text-sm text-[var(--m-text-muted)]">Training data details are not available for this model yet.</p>
        </Card>
      )}
    </DetailSection>
  );
