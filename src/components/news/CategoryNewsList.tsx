...
export default function CategoryNewsList({ categorySlug, categoryName }: CategoryNewsListProps) {
  ...
  const fetchNews = async (page: number = 1) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/news/category/${categorySlug}?page=${page}&limit=12`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {news.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-600">No {categoryName.toLowerCase()} articles found.</p>
        </div>
      )}
    </div>
  );
