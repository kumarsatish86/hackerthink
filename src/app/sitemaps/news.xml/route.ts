...
export async function GET() {
  try {
    ...
    // Check if news are included in sitemap
    const includeInSitemap = settings.include_in_sitemap || '';
    if (!includeInSitemap.includes('news')) {
      return new NextResponse('News are not included in sitemap', {
        status: 404,
        headers: {
          'Content-Type': 'text/plain',
        },
    ...
    for (const news of newsResult.rows) {
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/news/${news.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${new Date(news.publish_date || news.updated_at || new Date()).toISOString()}</lastmod>\n`;
