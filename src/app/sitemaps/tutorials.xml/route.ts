...
export async function GET() {
  try {
    ...
    // Check if tutorials are included in sitemap
    const includeInSitemap = settings.include_in_sitemap || '';
    if (!includeInSitemap.includes('tutorials')) {
      return new NextResponse('Tutorials are not included in sitemap', {
        status: 404,
        headers: {
          'Content-Type': 'text/plain',
        },
    ...
    // Add tutorial modules to sitemap
    for (const tutorial of tutorialsResult.rows) {
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/tutorials/${tutorial.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${new Date(tutorial.updated_at).toISOString()}</lastmod>\n`;
    ...
    console.error('Error generating tutorials sitemap:', error);
