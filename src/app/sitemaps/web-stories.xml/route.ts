import { NextResponse } from 'next/server';
import { Pool } from 'pg';
import { getSiteUrl } from '../../lib/getSiteUrl';

const pool = new Pool({
  host: process.env.DB_HOST || 'localhost',
  port: parseInt(process.env.DB_PORT || '5432'),
  user: process.env.DB_USER || 'postgres',
  password: process.env.DB_PASSWORD || 'Admin1234',
  database: process.env.DB_NAME || 'hackerthink',
});

// Specify Node.js runtime
export const runtime = 'nodejs';

export async function GET() {
  try {
    // Get base URL using our utility
    const baseUrl = getSiteUrl();
    
    // Get sitemap settings
    const settingsResult = await pool.query(`
      SELECT setting_key, setting_value
      FROM seo_settings
      WHERE setting_key IN (
        'generate_sitemap',
        'sitemap_change_frequency',
        'sitemap_priority'
      )
    `);
    
    // Convert to object format
    const settings = settingsResult.rows.reduce((acc, row) => {
      acc[row.setting_key] = row.setting_value;
      return acc;
    }, {});
    
    // If sitemap generation is disabled, return 404
    if (settings.generate_sitemap === 'false') {
      return new NextResponse('Sitemap generation is disabled', {
        status: 404,
        headers: {
          'Content-Type': 'text/plain',
        },
      });
    }
    
    // Default values if not set
    const changeFreq = settings.sitemap_change_frequency || 'weekly';
    const priority = settings.sitemap_priority || '0.7';
    
    // Get published stories
    // Try multiple possible table names since we don't know the exact schema
    let storiesResult;
    try {
      // First try web_stories table
      storiesResult = await pool.query(`
        SELECT slug, updated_at
        FROM web_stories
        WHERE published = true OR is_published = true
        ORDER BY updated_at DESC
      `);
    } catch (e) {
      // If that fails, try stories table
      try {
        storiesResult = await pool.query(`
          SELECT slug, updated_at
          FROM stories
          WHERE published = true OR is_published = true
          ORDER BY updated_at DESC
        `);
      } catch (e2) {
        // If both fail, provide empty results
        storiesResult = { rows: [] };
      }
    }
    
    // XML header
    let sitemapXml = '<?xml version="1.0" encoding="UTF-8"?>\n';
    sitemapXml += '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n';
    
    // Add each web story
    for (const story of storiesResult.rows) {
      const lastmod = new Date(story.updated_at).toISOString();
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/stories/${story.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${lastmod}</lastmod>\n`;
      sitemapXml += `    <changefreq>${changeFreq}</changefreq>\n`;
      sitemapXml += `    <priority>${priority}</priority>\n`;
      sitemapXml += `  </url>\n`;
    }
    
    // If no stories were found in the database, add example stories from the stories page
    if (storiesResult.rows.length === 0) {
      const exampleStories = [
        { slug: 'getting-started-with-linux' },
        { slug: 'linux-command-line-basics' },
        { slug: 'server-administration-best-practices' },
        { slug: 'bash-scripting-automation' },
        { slug: 'linux-networking-explained' },
        { slug: 'first-linux-environment' }
      ];
      
      for (const story of exampleStories) {
        sitemapXml += `  <url>\n`;
        sitemapXml += `    <loc>${baseUrl}/stories/${story.slug}</loc>\n`;
        sitemapXml += `    <lastmod>${new Date().toISOString()}</lastmod>\n`;
        sitemapXml += `    <changefreq>${changeFreq}</changefreq>\n`;
        sitemapXml += `    <priority>${priority}</priority>\n`;
        sitemapXml += `  </url>\n`;
      }
    }
    
    // Close the XML
    sitemapXml += '</urlset>';
    
    // Return the sitemap XML with proper content type
    return new NextResponse(sitemapXml, {
      status: 200,
      headers: {
        'Content-Type': 'application/xml',
        'Cache-Control': 'public, max-age=3600',
      },
    });
  } catch (error) {
    console.error('Error generating web-stories sitemap:', error);
    
    // Return a simple error-indicating sitemap in case of errors
    return new NextResponse(
      '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n</urlset>',
      {
        status: 500,
        headers: {
          'Content-Type': 'application/xml',
          'Cache-Control': 'public, max-age=60',
        },
      }
    );
  }
} 