...
export async function GET() {
  try {
    ...
    // Check if AI datasets are included in sitemap
    const includeInSitemap = settings.include_in_sitemap || '';
    if (!includeInSitemap.includes('ai-datasets')) {
      return new NextResponse('AI Datasets are not included in sitemap', {
        status: 404,
        headers: {
          'Content-Type': 'text/plain',
        },
    ...
    // Add AI datasets to sitemap
    for (const dataset of datasetsResult.rows) {
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/datasets/${dataset.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${new Date(dataset.updated_at || new Date()).toISOString()}</lastmod>\n`;
    ...
    console.error('Error generating AI datasets sitemap:', error);
