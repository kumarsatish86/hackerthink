...
export async function GET() {
  try {
    ...
    // Add scripts to sitemap
    for (const script of scriptsResult.rows) {
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/scripts/${script.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${new Date(script.updated_at).toISOString()}</lastmod>\n`;
    ...
    console.error('Error generating scripts sitemap:', error);
