...
export async function GET() {
  try {
    ...
    // Add lab exercises to sitemap
    for (const lab of labsResult.rows) {
      sitemapXml += `  <url>\n`;
      sitemapXml += `    <loc>${baseUrl}/lab-exercises/${lab.slug}</loc>\n`;
      sitemapXml += `    <lastmod>${new Date(lab.updated_at).toISOString()}</lastmod>\n`;
    ...
    console.error('Error generating lab exercises sitemap:', error);
