...
export async function DELETE(
  ...
  try {
    ...
    // Check if tutorial has sections
    const sectionsCount = await pool.query(
      'SELECT COUNT(*) FROM tutorial_sections WHERE tutorial_id = $1',
      [id]
    );

    if (parseInt(sectionsCount.rows[0].count) > 0) {
      return NextResponse.json(
        { message: 'Cannot delete tutorial that has sections. Please delete or reassign sections first.' },
        { status: 400 }
      );
    }
