...
// Main Web Story Creation Component
function WebStoryContent() {
  ...
  const [slides, setSlides] = useState([{
    id: '1',
    content: 'Welcome to your new web story!',
  ...
  useEffect(() => {
    ...
    const savedState = sessionStorage.getItem('web-story-new');
    if (savedState) {
      try {
        ...
        if (parsedState.method) setMethod(parsedState.method);
    ...
    if (methodParam === 'ai' || methodParam === 'manual') {
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      sessionStorage.setItem('web-story-new', JSON.stringify(stateToSave));
    }
  }, [slides, currentSlide, title, slug, coverImage, method, isInitialized]);
  ...
  }, [status, session?.user?.role]); // Removed router from dependencies
  ...
  const handleSaveStory = async () => {
    ...
    try {
      ...
      // Make API call to create the web story
      ...
      if (data.success) {
      setStatusMessage({ message: 'Web story saved successfully!', type: 'success' });
        
        // Clear saved state after successful save
        sessionStorage.removeItem('web-story-new');
        
      setTimeout(() => {
        router.push('/admin/content/web-stories');
      }, 1500);
      } else {
        setStatusMessage({ message: data.message || 'Failed to save story. Please try again.', type: 'error' });
      }
    } catch (err) {
      console.error('Error saving story:', err);
      setStatusMessage({ message: 'Failed to save story. Please try again.', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
                       ...
                       {slides[currentSlide].logo_url && (
                         ...
                         >
                           <img 
                             ...
                             style={{ opacity: slides[currentSlide].logo_opacity / 100 }}
                           />
                         </div>
                       )}
            ...
            <div className="lg:col-span-3 space-y-3 max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-lg p-3 border border-teal-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-teal-600">{slides[currentSlide].duration}s</span>
                </div>
                   <input
                     type="range"
                     min="1"
              ...
              <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-lg p-3 border border-violet-200">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <select
                      value={slides[currentSlide].logo_alignment}
                    ...
                    <div className="flex items-center space-x-1 min-w-0">
                      <span className="text-xs text-violet-700 whitespace-nowrap">{slides[currentSlide].logo_opacity}%</span>
                      <input
                        type="range"
                        min="0"
                        max="100"
                        value={slides[currentSlide].logo_opacity}
