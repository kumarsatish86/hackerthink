...
async function insertUserExpiryDateGeneratorTool() {
  const client = await pool.connect();
  
  try {
    console.log('Adding User Expiry Date Generator Tool to database...');
    ...
    if (checkResult.rows.length > 0) {
      console.log('✅ Tool already exists with slug: user-expiry-date-generator');
      return;
    }
    ...
    const insertResult = await client.query(
      ...
      [
        ...
        'calendar',
        '/tools/user-expiry-date-generator',
        true,
        new Date(),
        new Date()
      ]
    ...
    if (insertResult.rows.length > 0) {
      ...
      console.log(`   File Path: /tools/user-expiry-date-generator`);
...
// Run the function
insertUserExpiryDateGeneratorTool();
